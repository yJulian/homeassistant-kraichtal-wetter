    ...
    async def async_fetch_all(
        ...
        if include_forecast:
            try:
                ...
                data[ATTR_FORECAST] = _parse_forecast(html, data.get("meta", {}))
            except KraichtalWetterApiError as err:
                _LOGGER.warning("Could not update homepage forecast: %s", err)
                data[ATTR_FORECAST] = []

        data[ATTR_CURRENT_CONDITION] = _derive_current_condition(data)
        return data
...
def _parse_forecast(html: str, meta: dict[str, Any]) -> list[dict[str, Any]]:
...
def _derive_current_condition(data: dict[str, Any]) -> str | None:
    """Derive a Home Assistant weather condition."""
    radar = data.get(ATTR_RADAR) or {}
    if radar.get("raining_now"):
        return "rainy"

    forecast = data.get(ATTR_FORECAST) or []
    if forecast:
